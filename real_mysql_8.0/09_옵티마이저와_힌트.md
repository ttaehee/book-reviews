...
- [고급 최적화](#고급-최적화)
  - [MySQL의 join 방식들](#MySQL의-join방식들)
    - [1) Multi-Range Read(MRR)](#1-Multi-Range-Read(MRR))
    - [2) Nested Loop](#2-Nested-Loop)
    - [3) Block Nested Loop](#3-Block-Nested-Loop)
    - [4) Index Condition Pushdown](#4-Index-Condition-Pushdown)
    - [5) Use Index Exceptions](#5-Use-Index-Exceptions)
    - [6) Index Merge](#6-Index-Merge)
    - [7) Semi Join](#7-Semi-Join)
    - [8) Table Pull Out](#8-Table-Pull-Out)
    - [9) First Match](#9-First-Match)
    - [10) Loose Scan](#10-Loose-Scan)
    - [11) Materialization](#11-Materialization)
    - [12) Duplicated Weed Out](#12-Duplicated-Weed-Out)
    - [13) Condition Fanout Filter](#13-Condition-Fanout-Filter)
    - [14) Derived Merge](#14-Derived-Merge)
    - [15) Use Invisible Indexes](#15-Use-Invisible-Indexes)
    - [16) Skip Scan](#16-Skip-Scan)
    - [17) Hash Join](#17-Hash-Join)
    - [18) 인덱스 정렬 선호](#18-인덱스-정렬-선호)
- [쿼리 힌트](#쿼리-힌트)
  - [인덱스 힌트](#인덱스-힌트)
  - [옵티마이저 힌트](#옵티마이저-힌트)
- [💡 생각해볼 점](#-생각해볼-점)
...
- 정렬 처리 방법 : index 이용 시에는 이미 정렬되어 있기 때문에 별도 정렬 불필요    
  -> index 없으면? -> filesort라는 별도처리 통해 메모리에서 정렬됨
  
- MySQL은 sort buffer라는 별도의 메모리 공간을 받아서 정렬을 수행함   
  하지만 정렬해야 할 레코드가 메모리 공간보다 많아지는 경우, 임시 저장을 위해 디스크를 사용(multi-merge)
  - memory의 sort buffer에서 정렬 수행 -> 결과를 디스크에 임시 기록 -> 다음 레코드 가져와서 다시 정렬 -> 반복적으로 디스크에 임시 기록
    
  - sort buffer를 크게 설정하면 괜찮을까?
    - => 벤치마크 결과로는 큰차이 없음
...
- `투패스 정렬` : sort buffer에 정렬 대상 칼럼과 primary key 값만 담아서 정렬 수행 -> 정렬된 순서대로 다시 primary key로 table을 읽어 -> SELECT할 column 가져옴   
  -> table을 2번 읽어야함, 레코드의 크기가 또는 row 수가 클 때 효율적   
...
- 조건
  ...
  - WHERE 절에 driving table의 columd에 대한 조건이 있으면, 그 조건과 ORDER BY는 같은 index를 사용할 수 있어야함     

<br/>

**2) join의 driving table만 정렬**    

- `join 수행` 시, `레코의 건수 수 배 증가 + 레코드 단건의 크기도 증가`        
  => join 실행 전, `첫번째로 읽히는 테이블(driving table)의 record를 먼저 정렬` 후 -> `조인 실행`이 차선책!
...
**버퍼링 처리 방식**    
- ORDER BY, GROUP BY -> 모든 레코드 검색 후 작업이 필요 -> 스트리밍 불가 -> 응답속도가 느려짐   
  => LIMIT 걸어두어도 MySQL 서버가 해야하는 작업량 변화 없음 (네트워크에 전송되는 건수만 줄임) = 정렬이나 그루핑 작업이 대부분 느린 이유   

  => 단, index를 활용한 정렬은 스트리밍 형태의 처리 가능!   
...
- join의 driving table에 속한 column만 이용해 grouping 할 때, GROUP BY 컬럼으로 이미 index가 있다면 index만 차례대로 읽으면서 grouping -> 그 결과로 join 처리   
...
- MySQL 8.0에서도 GROUP BY + ORDER BY 같이 사용 -> 명시적으로 정렬 작업 실행
...
- DISTINCT는 조회하는 `모든 column의 조합`이 unique한 것들을 가져옴

<br/>

**MIN(), MAX(), COUNT() 등의 집합함수와 함께 사용하는 경우**    
- index 된 column에 대해 DISTINCT 처리 -> index를 full scan하거나 range scan = 임시 테이블 없이 최적화된 처리    
- index 아닌경우, 집합 함수 내에서 사용된 DISTINCT는 그 집합 함수의 인자로 전달된 컬럼값이 unique한 것들을 가져옴    
  ...
  => employees와 salaries 테이블 조인 결과에서 salary 컬럼의 값만 저장하기 위한 임시테이블 필요       
  임시테이블의 salary 컬럼은 unique index 생성됨   
  -> 레코드 건수가 많으면 느린 쿼리가 됨    

    - 참고) 집합 함수랑 사용 시, 임시 테이블이 필요 -> but Using temporary 미표시         
      -> 버그처럼 보이지만 모든 버전의 MySQL 서버에서 보여주는 실행 계획에서 표기되지 않음   
  ...
  => 각 column(s.salary, e.last_name)에 대한 임시 테이블을 만들어야해서 총 2개의 임시 테이블 사용       

<br/>

### 내부 임시 테이블 사용   
MySQL 엔진이 내부적인 가공을 위해 생성하는 임시 테이블   
- 다른 세션이나 쿼리에서 보거나 사용 불가   
- 쿼리 처리 후 자동 삭제 됨
- 메모리 임시 테이블 / 디스크 임시 테이블
...
## 고급 최적화
통계 정보와 옵티마이저 옵션 결합 -> 최적의 실행 계획 수립   

### MySQL의 join 방식들   
- join -> driving table에서 일치하는 레코드의 건수만큼 driven table 검색하면서 처리됨   
  ...
  - ex) driving table에서 일치하는 레코드가 1000건 -> driven table의 join 조건이 index 사용 못하면?     
    => driven table에서 연결되는 레코드 찾기위해 1000번의 full table scan 필요    
      = `driven table 검색 시, index 사용할 수 없는 쿼리는 상당히 느린`것   
...
#### 1) Multi-Range Read(MRR)
join buffer 사용해서 이게 가득찰 때 MySQL 엔진은 버퍼링된 레코드를 스토리지 엔진으로 한번에 요청   
-> 스토리지 엔진은 읽어야할 레코드들을 data page에 정렬된 순서로 접근   
-> disk의 data page 읽기를 최소화함   
이러한 읽기 방식을 MRR이라고 함   
MRR을 응용해서 실행되는 join 방식을 BKA(Batch Key Access)라고 함    

- index range scan에서 MMR이 사용 x되는 경우
...
- index range scan에서 MMR 기능 켜진(사용한) 경우   
  -> buffer 내에서 한번 데이터의 정렬이 일어나기 때문에 random I/O 감소   
...
- 중첩 for문 방식
  ...
  => 주의! 어떤 table이 먼저 access 되느냐에 따라 속도 차이 크게 발생
...
- MySQL 8.0.18부터 hash join algorithm으로 대체됨 (열심히 정독했는데 뒤에서 알랴주네 ㅎㅎ 정리는 앞에 해두기 ㅎ..)    
...
- ex) last_name과 first_name에 복합 인덱스(idx_lastname_firstname) + index_condition_pushdown 비활성화    
  ...
  - `Using where` : InnoDB 스토리지 엔진이 읽어서 반환해준 레코드가 index를 사용못하는 where 조건 부합여부 체크하는 과정 의미   
    - last_name = 'taehee' 조건으로 index range scan -> first_name LIKE '%im' 조건 부합 여부 체크 
      - 성능과 큰 관계가 있음
        - last_name = 'taehee' 에서 10만건 가져왔는데, first_name LIKE '%im' 까지 부합하는 애는 정작 1건이면?     
          -> 99,999건 왜읽었어..
        - 근데 first_name LIKE '%im' 조건 체크 때, 이미 한번 읽은 idx_lastname_firstname 인덱스의 first_name 컬럼 이용안하고 다시 테이블의 레코드를 읽어서 처리하는거지?       
          index의 first_name 컬럼 이용해서 비교했다면 불필요한 레코드는 테이블에서 안읽어도 됐을텐데?    
          => first_name LIKE '%im' 조건 체크 누가하느냐에 따라 index의 first_name 컬럼 이용할지 or 테이블의 first_name 컬럼 이용할지 결정됨   
          - index 비교하는 작업은 실제 InnoDB 스토리지 엔진이 수행
          - 테이블의 레코드에서 first_name 조건 비교는 MySQL 엔진이 수행
          - MySQL 5.5는 index 범위 제한 조건으로 사용 못하는 first_name 조건은 MySQL 엔진이 스토리지 엔진으로 전달안해줬음   

=> MySQL 5.6부터 index 범위 제한 조건으로 사용못해도 index에 포함된 column의 조건이 있다면 모두 같이 모아서 스토리지 엔진으로 전달 가능   
=> index 이용해 최대한 filtering까지해서 꼭 필요한 레코드에 대해서만 테이블 읽기 수행 가능      
=> index_condition_pushdown 활성화 -> `Using index condition`

<br/>

#### 5) Use Index Exceptions
인덱스 확장
- 세컨더리 인덱스에 추가된 PK를 활용여부를 결정하는 옵션
  
<br/>

#### 6) Index Merge   
- Index Merge 활용 -> 하나의 table에 대해 2개 이상의 index를 이용해 쿼리 처리
  - 쿼리에 사용된 각각의 조건이 서로 다른 index 사용 가능 & 조건을 만족하는 레코드 건수가 많을것으로 예상될 때 사용
  - 대부분의 optimizer는 table별로 하나의 index만 사용하여 실행 계획을 수립
...
- 2개 이상의 index를 이용해 나온 결과들을 `OR 연산자로 연결`할 경우 실행됨
  - `Using union(idx_1, idx_2)` = 두 index로 검색한 결과를 합집합했다는 뜻
- 합집합이라 중복되는 데이터 존재 가능 -> 두 결과를 우선순위큐 알고리즘을 통해 합치면서 중복제거함
  - 참고 1)      
    모든 secondary index는 clustering index인 PK를 포함하고 있으므로 같은값끼리는 PK로 정렬됨    
  - 참고 2)   
...
정렬 후 합집합
- 만약 별도의 정렬이 필요한 경우(ex) BETWEEN 같은), sort union 알고리즘 사용
...
#### 8) Table Pull Out   
semi join subquery에 사용된 테이블을 outer query로 끄집어낸 후 -> query를 join query로 재작성하는 형태의 최적화      
- ex) IN(subquery) -> join
  - 실행계획에는 표시되지 않음해당
    - 테이블들의 id 컬럼 값이 같은지 비교하기
...
- IN(subquery) -> EXISTS 형태로 튜닝한 것과 비슷한 방법으로 실행
  ...
  - subquery는 그 subquery가 참조하는 모든 outer table이 먼저 조회된 후 실행됨
    ...
    - 2) 이후에 사원번호를 기준으로 titles 테이블과 조인하여 from_date가 조건에 만족하는 레코드 찾음
  - 실행계획 : FirstMatch(e) = 테이블의 레코드에 대해 titles에 일치하는 레코드 1건만 찾으면 더 이상 검색하지 않는(단축 실행 경로)다는 것 의미    
    = 즉, EXISTS(subquery)와 동일하게 처리
    - 3) 일치하는 레코드가 없다면 사용자에게 반환되는 결과 없음 / 일치하는 첫 번째 레코드를 찾으면 더 이상 titles 테이블을 검색하지 않고 즉시 결과 반환
...
- ex) dept_emp 테이블에 존재하는 모든 부서 번호에 대해 부서 정보를 읽어 오는 쿼리
...
#### 11) Materialization
semi join에 사용된 subquery를 통째로 구체화해서 쿼리를 최적화하는 방식   
- 구체화 : 내부 임시 테이블을 생성한다는 의미
  - 실행계획에 MATERIALIZED 표시 / 사용하는 테이블은 2개인데 실행 계획은 3개(임시 테이블까지) 라인 출력
...
#### 12) Duplicated Weed Out
중복제거    
semi join subquery를 일반적인 inner join 쿼리로 바꿔서 실행하고 -> 중복된 레코드를 제거    
...
=> salaries 테이블의 primary key가 (emp_no + from_date) 일 때, salary가 15000 이상인 레코드 중 중복된 emp_no 발생 가능   
...
- ex) index (a,b) -> WHERE절에 B만 사용    
  -> a 컬럼의 조건이 있는것처럼 쿼리 최적화     
  => 만약 index의 선행 컬럼의 cardinality가 크면 skip scan 최적화를 비활성화 하기   

<br/>

#### 17) Hash Join   
- 보통 Nested Loop Join 사용 불가한 경우 해시 조인이 사용됨
  ...
  - Nested Loop Join은 첫 레코드 빠르게 받음 -> 최고 응답 속도 전략(OLTP에 유리)에 적합       
  - Hash Join은 첫 레코드 찾는게 오래걸림 & 마지막 레코드 찾는 시점은 더 빠름 -> 최고 스루풋 전략(OLAP에 유리)에 적합             
- Hash Join은 두 단계로 나뉘어 처리됨   
  - 빌드 단계 : join 대상 테이블 중 레코드 건수가 적은 테이블을 memory에 hash table 생성   
  - 프로브 단계 : 나머지 테이블의 레코드를 읽어서 hash table의 일치 레코드를 찾음   
...
하지만 optimizer가 ORDER BY절의 index에 가중치를 너무 부여하여 2번이 선택될수도      
