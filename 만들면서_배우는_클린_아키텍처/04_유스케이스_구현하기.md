...
- [유스케이스 둘러보기](#유스케이스-둘러보기)
  - [flow](#flow)
    - [입력 유효성 검증은 어디서 하는게 좋을까](#입력-유효성-검증)
    - [입력받기](#입력받기)
...
- 쿼리 서비스 구현(GetAccountBalanceService)
  - 아웃고잉 포트(LoadAccountPort)로 `쿼리를 전달`하는 것 외에 다른 일을 하지 않음  

- 프로덕트 맥락에서는 유스케이스로 간주되지는 않기 때문에    
  코드 상에서 읽기 전용 쿼리는 쓰기가 가능한 `Use Case(or Command)와 명확하게 구분`  

=> 이러한 `읽기 전용 유스케이스`는 `인커밍 전용 포트를 만들고` `Query Service에 구현`하는 것  

<br/><br/>

## 마무리    
- 유스케이스 간에 모델을 공유하는 것보다 별도 모델을 만들고, 엔티티를 매핑하는 등의 추가 작업을 해주자
  - 유스케이스를 명확하게 이해할 수 있고 장기적으로 유지보수하기도 더 쉬움      
